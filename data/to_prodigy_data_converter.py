...
if __name__ == '__main__':
    ...
    process_second_batch = False
    process_first_batch = False
    process_non_drug_batch = False
    process_minority_classes = True
    ...
    file_name_non_drug = "data_aact_sample/aact_neuro_samples_20240422_non_drug_biological_dietary_150.csv"
    file_name_minority_classes = "data_aact_sample/aact_neuro_samples__20240513_minority_classes.csv"

    if process_minority_classes:
        df_clin_trials_sample = pd.read_csv(file_name_minority_classes)
        ...
        save_text_json_to_csv(df_clin_trials_sample, "./data_for_prodigy"
                                                     "/ct_random_neurological_minority_classes_{}_{}.jsonl".format(len(df_clin_trials_sample), today_formatted))

    if process_non_drug_batch:
        df_clin_trials_sample = pd.read_csv(file_name_non_drug)
        df_clin_trials_sample['text_json'] = df_clin_trials_sample.apply(convert_text_to_json_official_combined_text,
                                                                         axis=1)

        save_text_json_to_csv(df_clin_trials_sample, "./data_for_prodigy"
                                                     "/ct_random_neurological_non_drug_{}_{}.jsonl".format(150, today_formatted))

    if process_second_batch:
        ...
        df_clin_trials_sample['text_json'] = df_clin_trials_sample.apply(convert_text_to_json_official_combined_text,
                                                                         axis=1)
        save_text_json_to_csv(df_clin_trials_sample, "./data_for_prodigy"
                                                     "/ct_random_neurological_{}_{}_second_batch.jsonl".format(500,
                                                                                                               today_formatted))
